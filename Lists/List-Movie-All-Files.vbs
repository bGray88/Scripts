...
Dim strDesktopPath : strDesktopPath = strUserFolder & "\Desktop\"
Const strTxtFileExt = ".txt"
...
' Read folder titles from within the base folder
...
If UBound(arrFolderTitles) >= 0 Then
	...
	For Each FolderName in arrFolderTitles
		If FolderExists(objSelectFolder & "\" & FolderName) Then
			WriteRecurseFolderFileName objTextStream, objSelectFolder & "\" & FolderName, 1
		End If
