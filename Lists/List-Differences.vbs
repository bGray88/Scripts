...
Do While strFileFolder = ""
	...
	End If
Loop
Dim strPathFileFolderName : strPathFileFolderName = GetFileFolderName(strFileFolder)
...
Dim strListTitle : strListTitle = "Differences (" & strPathFileFolderName & _
	"-" & strPathMediaFolderName & ")"
...
Dim arrListTitles : arrListTitles = ReadFileTitles(strFileFolder, intWinHeight, intWinWidth)
...
If UBound(arrListTitles) >= 0 And UBound(arrListMedia) >= 0 Then
	...
	objTextStream.WriteLine "File Listing Files" 
	objTextStream.WriteLine "-------------------------" & VbCrLf
	...
	Dim objTitle, objFile
	For Each objTitle in arrListTitles
		Dim blnFoundMatch : blnFoundMatch = false
		Dim strFileExt : strFileExt = GetExtension(objTitle)
		Dim strTitleName : strTitleName = Replace(objTitle, "." & strFileExt, "")
		For Each objFile in arrListMedia
			strFileExt = GetExtension(objFile)
			Dim strFileName : strFileName = Replace(objFile, "." & strFileExt, "")
	...
	objTextStream.WriteLine VbCrLf & "File Listing Media" 
	objTextStream.WriteLine "-------------------------" & VbCrLf
	
	intWriteCounter = 0
	intArrSize = UBound(arrListMedia) + 1
	Set objExplorer = WindowCreate(strAppTitle, intWinHeight, intWinWidth)
	HtmlProgress strAppTask, objExplorer, intWriteCounter, intArrSize
	For Each objTitle in arrListMedia
		blnFoundMatch = false
		strFileExt = GetExtension(objTitle)
		strTitleName = Replace(objTitle, "." & strFileExt, "")
		For Each objFile in arrListTitles
			strFileExt = GetExtension(objFile)
			strFileName = Replace(objFile, "." & strFileExt, "")
			If StrComp(strTitleName, strFileName, 1) = 0 Then
				blnFoundMatch = true
			End If
		Next
