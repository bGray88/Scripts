...
Dim objFileFolder : objFileFolder = ""
Do While objFileFolder = ""
	objFileFolder = BrowseForFolder( "Find the file directory", "" )
	If Not FolderExists(objFileFolder) Then
		MsgBox("The Folder Does Not Exist")
		objFileFolder = ""
	ElseIf FolderEmpty(objFileFolder) Then
		MsgBox("The Folder Is Empty")
		objFileFolder = ""
	End If
Loop

Dim strListTitle : strListTitle = "List Files " & GetFileFolderName(objFileFolder)
...
Dim arrListTitles : arrListTitles = ReadFileTitles(objFileFolder, intWinHeight, intWinWidth)
Dim arrListFolders : arrListFolders = ReadFolderTitles(objFileFolder, intWinHeight, intWinWidth)

Dim ItemTitle : ItemTitle = ""
If UBound(arrListTitles) >= 0 Or UBound(arrListFolders) >= 0 Then
	...
	If UBound(arrListFolders) >= 0 Then

		arrListFolders = BubbleSortListProgress(arrListFolders, intWinHeight, intWinWidth)

		intWriteCounter = 0
		intArrSize = UBound(arrListFolders) + 1
		Set objExplorer = WindowCreate(strAppTitle, intWinHeight, intWinWidth)
		HtmlProgress strAppTask, objExplorer, intWriteCounter, intArrSize
		For Each ItemTitle in arrListFolders
			HtmlProgress strAppTask, objExplorer, intWriteCounter, intArrSize
			If FolderExists(objFileFolder & "\" & ItemTitle) Then
				WriteRecurseFolderName objTextStream, objFileFolder & "\" & ItemTitle, 0
			End If
			intWriteCounter = intWriteCounter + 1
			HtmlProgress strAppTask, objExplorer, intWriteCounter, intArrSize
		Next
