...
# Directory tree names
!define ALTPATHNAME "DIRECTORY_NAME\"
...
Section
	...
	# Change output path
	SetOutPath "$INSTDIR\${ALTPATHNAME}"
	
	CopyFiles /SILENT "$EXEDIR\${BASEFILENAME}.7z" "$OUTDIR"
	Nsis7z::Extract "$OUTDIR\${BASEFILENAME}.7z"
	Delete "$OUTDIR\${BASEFILENAME}.7z"
	
	# Create Shortcut(s)
	# -------------------------------------------	
	# Define Start Menu shortcut(s)
	CreateDirectory "$SMPROGRAMS\${SOFTWARETYPE}\${BASEFILENAME}\"
	CreateShortCut "$SMPROGRAMS\${SOFTWARETYPE}\${BASEFILENAME}\Uninstall.lnk" "$OUTDIR\Uninstall.exe"
	CreateShortCut "$SMPROGRAMS\${SOFTWARETYPE}\${BASEFILENAME}\${BASEFILENAME}.lnk" \
		'"$OUTDIR\${EXEFILENAME}"' "" "$OUTDIR\${ICONFILENAME}"
