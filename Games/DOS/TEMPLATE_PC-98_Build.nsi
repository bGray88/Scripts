...
Section
	...
	SetOutPath "$INSTDIR\DOSBox-X"
	
	# Define Start Menu shortcut(s)
	CreateDirectory "$SMPROGRAMS\PC-98\${INSTALLFILENAME}\"
	CreateShortCut "$SMPROGRAMS\PC-98\${INSTALLFILENAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe"
	CreateShortCut "$SMPROGRAMS\PC-98\${INSTALLFILENAME}\${INSTALLFILENAME}.lnk" \
		'"$OUTDIR\dosbox-x.exe"' \
...
Section "Uninstall"
	...
	Delete "$SMPROGRAMS\PC-98\${INSTALLFILENAME}\*.*"
	RMDir /r "$SMPROGRAMS\PC-98\${INSTALLFILENAME}"
	RMDir "$SMPROGRAMS\PC-98"
